	...
	Element ***Array)
{
	...
	*Array =
	...
	while (fgets(buffer, MAX_ITEM_SIZE, infile))
	{
		...
		//(*Array)[i]->data = malloc(strlen(buffer) + 1);
		(*Array)[i]->data = malloc(sizeof(int));
...
void ShowArray(Element **Array, int Items, CompFunc Compare)
{
	...
	for (i = 0; i < Items; i++)
	{
		...
		printf("%3d: %d", i, Array[i]->data);    //
		//column += 5 + strlen(Array[i]->data);
